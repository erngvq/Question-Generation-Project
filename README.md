...
4) Now `src.py` is setup to train the model

With a little modification to the forward pass (in particular taking the question on how to output the actual
output of the model in the print statements in the **train** function) a system can be setup to reliably test the
model of the test set that is created during pre_processing. The data the is put into the train function can
simply be changed by changing the path that is put into it in the **build_model_and_train_function**.
