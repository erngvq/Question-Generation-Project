        ...
        try:
            ...
            for j in range(0, BATCH_SIZE):
                ...
                if i % 1000 == 0:
                    ...
                    # Prints the loss but that is later in the code
...
def build_model_and_train():
    ...
    # The hidden size is a parameter of any RNN, it can be thought of the space that BERT words are projected
    # into.That is a bit abstract, but, it is essentially where the model learns to represent the sentence at a
    # particular word. If this space is very large, it is possible for the model not to learn well as it will not
    # find important details and instead just encapsulate everything as-is. If this space is very small the model
    # might be unable to learn as it simply cannot find what is important in the data. The size 600 here comes from
    # the original paper and is what they found to be best.

    encoder = BiAttnGRUEncoder(input_size=768 + 3, hidden_size=600)
    encoder.init_weights()

    # The hidden size is notably doubled here due to the encoder being bi-directional. The decoder also does not
    # take BIO tags as input. Instead it takes the previously predicted word, or in the case of teacher forcing,
    # the ground truth.
    ...
    # This creates a dataset compatible with pytorch that auto-shuffles and we do not have to 
    # worry about indexing errors
